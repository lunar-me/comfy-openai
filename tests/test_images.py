...
class FakeComfyClient:
    ...
    async def upload_image(self, data, filename, overwrite=False, type="input", subfolder=""):
...
def test_edit_uploads_image_and_returns_result():
    fake_storage = FakeStorage()
    app.dependency_overrides[get_executor] = lambda: FakeExecutor()
    app.dependency_overrides[get_storage] = lambda: fake_storage
    ...
    try:
        response = client.post(
            ...
            files={
                "image": ("original.png", b"\x89PNG-original-bytes", "image/png"),
            },
        )
    finally:
        app.dependency_overrides.clear()
